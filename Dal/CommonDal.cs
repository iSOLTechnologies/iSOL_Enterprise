...
where s.Status=1 and p.Guid=@Guid";
  ...
  //                  queryAppend = String.Format(DtQuery, xb.ToString(), sb.ToString());
        ...
        public dynamic GetBaseDocItemServiceList(string DocId , int BaseType)
        {
            ...
            SqlDataAdapter sda = new SqlDataAdapter("select Id,LineNum,ItemCode,ItemName,Quantity,DiscPrcnt,Price,VatGroup,UomCode,CountryOrg,Dscription,AcctCode,OpenQty,LineTotal from " + table+" where id = '" + DocId + "'", conn);
            sda.Fill(ds);
