...
namespace iSOL_Enterprise.Dal
{
    public class APCreditMemoDal
    {
        ...
        {
            string GetQuery = "select * from ORPC";
                ...
                try
                {
                    ...
                    if (model.ListItems != null)
                    {
                        ...
                        foreach (var item in model.ListItems)
                        {
                            ...
                            if (item.BaseEntry != "" && item.BaseEntry != null)
                            {
                                ...
                                string Updatequery = @"Update PCH1 set OpenQty =OpenQty - " + item.QTY + " where Id =" + item.BaseEntry + "and LineNum =" + item.BaseLine;
                            ...
                            string RowQueryItem = @"insert into RPC1(Id,LineNum,BaseRef,BaseEntry,BaseLine,BaseQty,ItemName,Price,LineTotal,OpenQty,ItemCode,Quantity,DiscPrcnt,VatGroup , UomCode,UomEntry ,CountryOrg)
                                              values(" + Id + ","
                                                + LineNo + ",'"
                                                + item.BaseRef + "',"
                                                + item.BaseEntry + ","
                    ...
                    else if (model.ListService != null)
                    {
                        ...
                        foreach (var item in model.ListService)
                        {
                            ...
                            string RowQueryService = @"insert into RPC1(Id,LineNum,BaseRef,BaseEntry,BaseLine,LineTotal,Dscription,AcctCode,VatGroup)
                                                  values(" + Id + ","
                                                     + LineNo + ",'"
                                                     + item.BaseRef2 + "',"
                                                     + item.BaseEntry2 + ","
                    ...
                    if (model.ListItems != null)
                    {
                        foreach (var item in model.ListItems)
                        {
                            ...
                            if (item.LineNum != "" && item.LineNum != null)
                            {
                                ...
                                string oldDataQuery = @"select BaseEntry,BaseLine,Quantity from RPC1 where Id=" + model.ID + " and LineNum=" + item.LineNum + " and OpenQty <> 0";
                                ...
                                {
                                    string Updatequery = @"Update PCH1 set OpenQty =(OpenQty + " + docRowModel.Quantity + ") - " + item.QTY + " where Id =" + docRowModel.BaseEntry + "and LineNum =" + docRowModel.BaseLine;
                                    int res = SqlHelper.ExecuteNonQuery(tran, CommandType.Text, Updatequery).ToInt();
                                    if (res <= 0)
                                    {
                                        tran.Rollback();
