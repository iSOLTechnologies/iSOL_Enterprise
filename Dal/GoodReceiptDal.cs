            ...
            try
            {
                ...
                try
                {
                    ...
                    if (model.HeaderData != null)
                    {
                        ...
                        string HeadQuery = @"insert into OPDN(Id,Series,DocType,Guid,CardCode,DocNum,CardName,CntctCode,DocDate,NumAtCard,DocDueDate,DocCur,TaxDate , GroupNum,DocTotal , SlpCode , Comments) 
                                           values(" + Id + ","
        ...
        public bool EditGoodReceipt(string formData)
        {
            try
            {
                ...
                try
                {
                    ...
                    var Status = CommonDal.Check_IsNotEditable("PCH1", Convert.ToInt32(model.ID)) == false ? "Open" : "Closed";
                    if (Status == "Closed")
                    {
                        tran.Rollback();
                        return false;
