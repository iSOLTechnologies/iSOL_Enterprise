$(document).ready(function () {
    ...
    function AddGroup() {
        $.ajax({
            url: "/_Drivercardlog/AdddriverDevice", 
            dataType: 'json',
            type: "POST",
            data: FieldsData(),
        ...
        if ($('#cardno').val() == "" || $('#cardno').val() == null) {
            ...
            setInterval(function () {
                if (!flag) {
                    ...
                    toastr.warning("Driver card no is Mendatory");
                }
            }, 1);
        }
        else {
            if ($('#cardno').val().match(letters)) {
                ...
                setInterval(function () {
                    if (!flag1) {
                        ...
                        toastr.warning('Please type alphanumeric characters only in Driver card no');
                    }
                }, 1);
            }
        }
        if ($('#cardno').val().length > 5) {
            ...
            setInterval(function () {
                if (!flag2) {
                    ...
                    toastr.warning("Driver card no must be less than 6 digits");
                }
            }, 1);
        }
