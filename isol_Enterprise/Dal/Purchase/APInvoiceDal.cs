            ...
            try
            {
                ...
                try
                {
                    ...
                    if (model.HeaderData != null)
                    {
                        ...
                        model.HeaderData.Series = model.HeaderData.Series == null || model.HeaderData.Series == "" ? "NULL" : Convert.ToInt32(model.HeaderData.Series);
                        ...
                        string HeadQuery = @"insert into OPCH(Id,Series,DocType,Guid,CardCode,DocNum,CardName,CntctCode,DocDate,NumAtCard,DocDueDate,DocCur,TaxDate , GroupNum,DocTotal , SlpCode ,DiscPrcnt,
                    ...
                    if (model.ListItems != null)
                    {
                        ...
                        foreach (var item in model.ListItems)
                        {
                            ...
                            string RowQueryItem = @"insert into PCH1(Id,LineNum,WhsCode,BaseRef,BaseEntry,BaseLine,BaseQty,BaseType,ItemName,Price,LineTotal,OpenQty,ItemCode,Quantity,DiscPrcnt,VatGroup , UomCode,
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if (model.HeaderData != null)
                        {
                            ...
                            model.HeaderData.TypeDetail = model.HeaderData.TypeDetail == null || model.HeaderData.TypeDetail == null ? "NULL" : Convert.ToDecimal(model.HeaderData.TypeDetail);
                            model.HeaderData.ProductionOrderNo = model.HeaderData.ProductionOrderNo == "" ? "NULL" : Convert.ToDecimal(model.HeaderData.ProductionOrderNo);
                            model.HeaderData.ChallanNo = model.HeaderData.ChallanNo == "" ? "NULL" : Convert.ToDecimal(model.HeaderData.ChallanNo);
                            model.HeaderData.DONo = model.HeaderData.DONo == "" ? "NULL" : Convert.ToDecimal(model.HeaderData.DONo);
                            model.HeaderData.SalesOrderCode = model.HeaderData.SalesOrderCode == "" ? "NULL" : Convert.ToInt32(model.HeaderData.SalesOrderCode);
                            ...
                            if (isApproved == 0)
                            {
                                ApprovalModel approvalModel = new()
                                {
                                    ...
                                    ObjectCode = ObjectCode,
                                    DocEntry = model.ID,
                                    DocNum = SqlHelper.ExecuteScalar(tran, CommandType.Text, @"select DocNum from OPCH where id=" + model.ID).ToString(),
                                    Guid = SqlHelper.ExecuteScalar(tran, CommandType.Text, @"select GUID from OPCH where id=" + model.ID).ToString()
                                };
                                bool response = dal.AddApproval(tran, approvalModel);
                                if (!response)
                                    return false;
                        ...
                        if (model.ListItems != null)
                        {
                            ...
                            foreach (var item in model.ListItems)
                            {
                                ...
                                else
                                {
                                    ...
                                    string RowQueryItem = @"insert into PCH1(Id,LineNum,WhsCode,ItemName,Price,LineTotal,OpenQty,ItemCode,Quantity,DiscPrcnt,VatGroup , UomCode,
                                                            UomEntry,SaleOrderCode,SaleOrderDocNo,PreCostingTowelCode,AccessoriesType ,CountryOrg)
                                              values(" + model.ID + ","
                                                     + LineNo + ",'"
                                                     + item.Warehouse + "','"                                                     
