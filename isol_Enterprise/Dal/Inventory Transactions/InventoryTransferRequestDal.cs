...
namespace iSOL_Enterprise.Dal.Inventory_Transactions
{
    public class InventoryTransferRequestDal
    {
        ...
        public int GetId(string guid)
        {
            ...
            return Convert.ToInt32(SqlHelper.ExecuteScalar(SqlHelper.defaultDB, CommandType.Text, "select Id from OWTQ where GUID ='" + guid.ToString() + "'"));
        ...
        public dynamic GetHeaderOldData(int ItemID)
        {
            try
            {
                ...
                string headerQuery = @"select MySeries,DocNum,Series,CardCode,CardName,DocDate,CntctCode,DocDueDate,TaxDate,Address,GroupNum,Filler,ToWhsCode,
                                     SlpCode,JrnlMemo,Comments,PurchaseType,TypeDetail,ProductionOrderNo,ChallanNo,DONo,SaleOrderNo From OWTQ where Id =" + ItemID;
        ...
        public ResponseModels AddInventoryTransferRequest(string formData)
        {
            ...
            try
            {
                ...
                if (model.HeaderData != null)
                {
                    ...
                    string HeadQuery = @"insert into OWTQ (Id,Guid,MySeries,DocNum,Series,DocDate,CntctCode,DocDueDate,GroupNum,TaxDate,Address,ShipToCode,CardName,CardCode,Comments,
                                        JrnlMemo,Filler,ToWhsCode,PurchaseType,TypeDetail,ProductionOrderNo,ChallanNo,DONo,SaleOrderNo) 
                                        values(@Id,@Guid,@MySeries,@DocNum,@Series,@DocDate,@CntctCode,@DocDueDate,@GroupNum,@TaxDate,@Address,@ShipToCode,@CardName,@CardCode,@Comments,
                                        @JrnlMemo,@Filler,@ToWhsCode,@PurchaseType,@TypeDetail,@ProductionOrderNo,@ChallanNo,@DONo,@SaleOrderNo)";
                          ...
                          values(@Id,@LineNum,@BaseRef,@BaseEntry,@BaseLine,@ItemCode,@Dscription,@WhsCode,@FromWhsCod,@Quantity,@UomEntry,@UomCode,@BaseQty,@OpenQty)";
                            ...
                            #region UpdateWarehouse&GenerateLog
                                ...
                                if (model.Batches != null)
                                {
                                    ...
                                    bool responseBatch = cdal.TransferRequestBatches(tran, model.Batches, item.ItemCode.ToString(), LogEntry, item.Warehouse.ToString(), LineNum);
                                    if (!responseBatch)
                                    {
                                        tran.Rollback();
                                        response.isSuccess = false;
                ...
                if (res1 > 0)
                {
                    ...
                    response.Message = "Inventory Transfer Request Added Successfully !";
