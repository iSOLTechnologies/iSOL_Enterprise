                        ...
                        if (model.HeaderData != null)
                        {
                            ...
                            if (isApproved == 0)
                            {
                                ApprovalModel approvalModel = new()
                                {
                                    ...
                                    ObjectCode = ObjectCode,
                                    DocEntry = model.ID,
                                    DocNum = SqlHelper.ExecuteScalar(tran, CommandType.Text, @"select DocNum from ODLN where id=" + model.ID).ToString(),
                                    Guid = SqlHelper.ExecuteScalar(tran, CommandType.Text, @"select GUID from ODLN where id=" + model.ID).ToString()
                                };
                                bool response = dal.AddApproval(tran, approvalModel);
                                if (!response)
                                    return false;
                            ...
                            foreach (var item in model.ListItems)
                            {
                                ...
                                if (item.LineNum != "" && item.LineNum != null)
                                {
                                    ...
                                    if (OpenQty > 0)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if (Convert.ToDecimal( item.QTY )!= Convert.ToDecimal(item.OldQty))
                                            {
                                                ...
                                                if (ItemData.Data.ManBtchNum == "Y")
                                                {
                                                    ...
                                                    if (dal.ReverseOutTransaction(tran, Convert.ToInt32(model.ID), Convert.ToInt32(item.LineNum),15))
                                                    {
                                        ...
                                        if (!SqlHelper.SAPIntegration)
                                        {
                                            string OldQtyQuery = @"select DocQty from OITL where DocEntry="+model.ID+" and DocLine = " + item.LineNum + "and DocType =15";
