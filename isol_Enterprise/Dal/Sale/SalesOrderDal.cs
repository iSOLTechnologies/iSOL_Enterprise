        ...
        public List<SalesQuotation_MasterModels> GetSaleOrderData()
        {
            ...
            using (var rdr = SqlHelper.ExecuteReader(SqlHelper.defaultDB, CommandType.Text, GetQuery))
            {
                while (rdr.Read())
                {
                    ...
                    models.DocStatus = CommonDal.Check_IsNotEditable("RDR1", rdr["Id"].ToInt()) == false ? "Open" : "Closed";
                ...
                try
                {
                    ...
                    if (model.ListItems != null)
                    {
                        ...
                        foreach (var item in model.ListItems)
                        {
                            ...
                            string RowQueryItem = @"insert into RDR1(Id,WhsCode,LineNum,BaseRef,BaseEntry,BaseLine,BaseQty,BaseType,ItemName,Price,LineTotal,OpenQty,ItemCode,Quantity,DiscPrcnt,VatGroup , UomCode,UomEntry ,CountryOrg)
                                              values(" + Id + ",'"
                                              + item.Warehouse + "',"
                                              + LineNo + ",'"
                                              + item.BaseRef + "',"
        ...
        public bool EditSaleOrder(string formData)
        {
            try
            {
                ...
                try
                {
                    ...
                    if (Status == "Closed")
                    {
                        string HeadQuery = @" Update ORDR set NumAtCard = '" + model.HeaderData.NumAtCard + "'" +
                        ...
                        if (model.HeaderData != null)
                        {
                            ...
                            if (isApproved == 0)
                            {
                                ApprovalModel approvalModel = new()
                                {
                                    ...
                                    ObjectCode = ObjectCode,
                                    DocEntry = model.ID,
                                    DocNum = SqlHelper.ExecuteScalar(tran, CommandType.Text, @"select DocNum from ORDR where id=" + model.ID).ToString(),
                                    Guid = SqlHelper.ExecuteScalar(tran, CommandType.Text, @"select GUID from ORDR where id=" + model.ID).ToString()
                                };
                                bool response = dal.AddApproval(tran, approvalModel);
                                if (!response)
                                    return false;
                        ...
                        if (model.ListItems != null)
                        {
                            ...
                            foreach (var item in model.ListItems)
                            {
                                ...
                                else
                                {
                                    ...
                                    string RowQueryItem = @"insert into RDR1(Id,WhsCode,LineNum,ItemName,Price,LineTotal,OpenQty,ItemCode,Quantity,DiscPrcnt,VatGroup , UomCode,UomEntry ,CountryOrg)
                                              values(" + model.ID + ",'"
                                                      + item.Warehouse + "',"
                                                      + LineNo + ",'"
                                                      + item.ItemName + "',"
