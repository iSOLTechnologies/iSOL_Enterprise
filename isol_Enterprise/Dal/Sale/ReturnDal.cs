...
namespace iSOL_Enterprise.Dal.Sale
{
    public class ReturnDal
    {
        ...
        {
            string GetQuery = "select * from ORDN order by id DESC";
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (model.HeaderData != null)
                    {
                        ...
                        string HeadQuery = @"insert into ORDN(Id,Series,DocType,Guid,CardCode,DocNum,CardName,CntctCode,DocDate,NumAtCard,DocDueDate,DocCur,TaxDate , GroupNum ,DocTotal, SlpCode ,DiscPrcnt,
                    ...
                    else if (model.ListService != null)
                    {
                        ...
                        foreach (var item in model.ListService)
                        {
                            ...
                            string RowQueryService = @"insert into RDN1(Id,LineNum,BaseRef,BaseEntry,BaseLine,BaseType,LineTotal,Dscription,AcctCode,VatGroup)
                                                  values(" + Id + ","
                                                     + LineNo + ",'"
                                                     + item.BaseRef2 + "',"
                                                     + item.BaseEntry2 + ","
        ...
        public bool EditReturn(string formData)
        {
            try
            {
                ...
                try
                {
                    ...
                    if (Status == "Closed")
                    {
                        string HeadQuery = @" Update ORDN set NumAtCard = '" + model.HeaderData.NumAtCard + "'" +
                        ...
                        if (model.HeaderData != null)
                        {
                            ...
                            if (isApproved == 0)
                            {
                                ApprovalModel approvalModel = new()
                                {
                                    ...
                                    ObjectCode = ObjectCode,
                                    DocEntry = model.ID,
                                    DocNum = SqlHelper.ExecuteScalar(tran, CommandType.Text, @"select DocNum from ORDN where id=" + model.ID).ToString(),
                                    Guid = SqlHelper.ExecuteScalar(tran, CommandType.Text, @"select GUID from ORDN where id=" + model.ID).ToString()
                                };
                                bool response = dal.AddApproval(tran, approvalModel);
                                if (!response)
                                    return false;
                            ...
                            foreach (var item in model.ListItems)
                            {
                                ...
                                if (item.LineNum != "" && item.LineNum != null)
                                {
                                    ...
                                    if (OpenQty > 0)
                                    {
                                        ...
                                        else
                                        {
                                            if (Convert.ToDecimal(item.QTY) != Convert.ToDecimal(item.OldQty))
                                            {
                                                ...
                                                if (ItemData.Data.ManBtchNum == "Y")
                                                {
                                                    ...
                                                    if (dal.ReverseOutTransaction(tran, Convert.ToInt32(model.ID), Convert.ToInt32(item.LineNum), 16))
                                                    {
                                        ...
                                        if (!SqlHelper.SAPIntegration)
                                        {
                                            string OldQtyQuery = @"select DocQty from OITL where DocEntry=" + model.ID + " and DocLine = " + item.LineNum + "and DocType =16";
                                ...
                                else
                                {
                                    ...
                                    string RowQueryItem = @"insert into RDN1(Id,WhsCode,LineNum,ItemName,Price,LineTotal,OpenQty,ItemCode,Quantity,DiscPrcnt,VatGroup , UomCode,UomEntry ,CountryOrg)
                                              values(" + model.ID + ",'"
                                               + item.Warehouse + "',"
                                                + LineNo + ",'"                                                
                                                + item.ItemName + "',"
